...
2.  注册或登录您的账号
3.  点击右上角的「New Repository」来新建远程仓库 ![](Images/15fe1eecb7d0d26d)
4.  进入仓库设置页面填写信息：其中 ① 是你的仓库名，这个仓库名同样会被 GitHub 设置为你的仓库的根目录的名称；② 是为 `.gitignore` 设置项目类型，`.gitignore` 是 Git 仓库中的一个特殊的文本文件，它里面记录了你不希望提交到仓库的目录和文件的名称或类型，例如你的 `/build` 目录；把 ① 和 ② 填好之后，就可以点 ③ 来完成远程仓库的创建了 ![](Images/15fe1eecb9a7cb5c)

创建完的远程仓库大概长这样：

![](Images/15fe1eecc08acc7a)

点击右边的「Clone or download」，然后把仓库的 `clone` 地址复制到剪贴板：

![](Images/15fe1eecbe9d91ba)
...
![](Images/15fe1eed08a0b366)

![](Images/15fe1eed0acc2a32)
...
![](Images/15fe1eed765d27b4)

嗯删掉以后脸不烫了，赶紧把它提交上去。提交之前先看看文件状态是个好习惯：

    git status

![](Images/15fe1eed76a5786c)
...
![](Images/15fe1eed7d63ed73)

好，最后一步，`commit`：

    git commit

![](Images/15fe1eed8407958e)

这时再看看 `log`，你已经有三条 `commit`s 了：

    git log

![](Images/15fe1eed8a40b0d1)
